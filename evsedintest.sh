...
evsedintest() {
			...
			if [[ $evsecon == "modbusevse" ]]; then 
				if [[ $modbusevsesource = *virtual* ]]; then
					if ps ax |grep -v grep |grep "socat pty,link=$modbusevsesource,raw tcp:$modbusevselanip:26" > /dev/null; then
						...
						socat pty,link=$modbusevsesource,raw tcp:$modbusevselanip:26 &
					fi
				else
					echo "echo" > /dev/null
				fi
				sleep 1
				python runs/evsewritembusdev.py $modbusevsesource $modbusevseid 1000 9
				sleep 1
				evsedinstat=$(python runs/readmodbus.py $modbusevsesource $modbusevseid 1000 1)
				...
				sleep 1
				python runs/evsewritembusdev.py $modbusevsesource $modbusevseid 1000 0
				sleep 1
			fi
			if [[ $evsecon == "masterethframer" ]]; then
				python runs/evsewritembusethframerdev.py 192.168.193.18 1 1000 9
				sleep 1
				evsedinstat=$(python runs/readmodbusethframer.py 192.168.193.18 1 1000 1)
				if [[ $evsedinstat == "[9]" ]]; then
					openwbDebugLog "MAIN" 0 "EVSE LP1 Prüfung erfolgreich"
					echo "erfolgreich" > ramdisk/evsedintestlp1
				else
				...
				sleep 1
				python runs/evsewritembusethframerdev.py 192.168.193.18 1 1000 0
				sleep 1
			fi
		else
		...
		if [[ $evsecons1 == "modbusevse" ]]; then
			if [[ $evsesources1 = *virtual* ]]; then
				if ps ax |grep -v grep |grep "socat pty,link=$evsesources1,raw tcp:$evselanips1:26" > /dev/null; then
					...
					socat pty,link=$evsesources1,raw tcp:$evselanips1:26 &
				fi
			else
				echo "echo" > /dev/null
			fi
			sleep 1
			python runs/evsewritembusdev.py $evsesources1 $evseids1 1000 9
			sleep 1
			evsedinstat=$(python runs/readmodbus.py $evsesources1 $evseids1 1000 1)
			if [[ $evsedinstat == "9" ]]; then
			...
			sleep 1
			python runs/evsewritembusdev.py $evsesources1 $evseids1 1000 0
			sleep 1
		else
	...
	if [[ $evsedintestlp3 == "ausstehend" ]]; then
		if [[ $evsecons2 == "modbusevse" ]]; then
			...
			sleep 1
			python runs/evsewritembusdev.py $evsesources2 $evseids2 1000 9
			sleep 1
			evsedinstat=$(python runs/readmodbus.py $evsesources2 $evseids2 1000 1)
			if [[ $evsedinstat == "9" ]]; then
			...
			sleep 1
			python runs/evsewritembusdev.py $evsesources2 $evseids2 1000 0
			sleep 1
		else
	...
	if [[ $evseausgelesen == "0" ]]; then
		if [ $evsecon == "modbusevse" ] || [ $evsecon == "masterethframer" ]; then
			if [[ $evsecon == "modbusevse" ]]; then 
				...
				sleep 1
				evselp12000=$(python runs/readmodbus.py $modbusevsesource $modbusevseid 2000 1)
				echo $evselp12000 > /var/www/html/openWB/ramdisk/progevsedinlp12000
				sleep 1
				evselp12007=$(python runs/readmodbus.py $modbusevsesource $modbusevseid 2007 1)
				echo $evselp12007 > /var/www/html/openWB/ramdisk/progevsedinlp12007
				sleep 1
			fi
			if [[ $evsecon == "masterethframer" ]]; then
				sleep 1
				evselp12000=$(python runs/readmodbusethframer.py 192.168.193.18 1 2000 1)
				echo $evselp12000 > /var/www/html/openWB/ramdisk/progevsedinlp12000
				sleep 1
				evselp12007=$(python runs/readmodbusethframer.py 192.168.193.18 1 2007 1)
				echo $evselp12007 > /var/www/html/openWB/ramdisk/progevsedinlp12007
				sleep 1
			fi
		fi
		...
		if [[ $evsecons1 == "modbusevse" ]]; then
			...
			evselp22000=$(python runs/readmodbus.py $evsesources1 $evseids1 2000 1)
			echo $evselp22000 > ramdisk/progevsedinlp22000
			sleep 1
			evselp22007=$(python runs/readmodbus.py $evsesources1 $evseids1 2007 1)
	...
	if [[ $progevselp1 == "1" ]]; then
		if [ $evsecon == "modbusevse" ] || [ $evsecon == "masterethframer" ]; then
			if [[ $evsecon == "modbusevse" ]]; then 
				...
				python runs/evsewritembusdev.py $modbusevsesource $modbusevseid 2000 $lp12000
				sleep 1
				lp12007=$(<ramdisk/progevsedinlp12007)
				python runs/evsewritembusdev.py $modbusevsesource $modbusevseid 2007 $lp12007
			fi
			if [[ $evsecon == "masterethframer" ]]; then
				lp12000=$(<ramdisk/progevsedinlp12000)
				python runs/evsewritembusethframerdev.py 192.168.193.18 1 2000 $lp12000
				sleep 1
				lp12007=$(<ramdisk/progevsedinlp12007)
				python runs/evsewritembusethframerdev.py 192.168.193.18 1 2007 $lp12007
			fi
		fi
		...
		if [[ $evsecons1 == "modbusevse" ]]; then 
			if [[ $modbusevsesource = *virtual* ]]; then
				if ps ax |grep -v grep |grep "socat pty,link=$evsesources1,raw tcp:$modbusevselanips1:26" > /dev/null; then
					...
					socat pty,link=$evsesources1,raw tcp:$modbusevselanips1:26 &
				fi
			else
				echo "echo" > /dev/null
			fi
			...
			python runs/evsewritembusdev.py $evsesources1 $evseids1 2000 $lp22000
			sleep 1
			lp22007=$(<ramdisk/progevsedinlp22007)
			python runs/evsewritembusdev.py $evsesources1 $evseids1 2007 $lp22007
		fi
...
evsemodbuscheck() {
	if [[ $evsecon == "modbusevse" ]]; then
		...
		if [[ $evsedinstat == "$llalt" ]]; then
			...
			python runs/evsewritembusdev.py $modbusevsesource $modbusevseid 1000 $llalt
		fi
	fi
	if (( lastmanagement == 1 )); then 
		if [[ $evsecons1 == "modbusevse" ]]; then
			...
			if [[ $evsedinstat == "$llalts1" ]]; then
				...
				python runs/evsewritembusdev.py $evsesources1 $evseids1 1000 $llalts1
			fi
		fi
		if (( lastmanagements2 == 1 )); then
			if [[ $evsecons2 == "modbusevse" ]]; then
				...
				if [[ $evsedinstat == "$llalts2" ]]; then
					...
					python runs/evsewritembusdev.py $evsesources2 $evseids2 1000 $llalts2
				fi
			fi
		fi
	fi
