...
ledsteuerung() {
	...
	if (( slavemode == 1 ));then
		...
		if [[ $ledstatus != $slaveLedStatus ]]; then
			python runs/leds.py $slaveLedStatus &
		...
		if (( lademodus == 4 )); then
			if [[ $ledstatus != $ledstandby ]]; then
				...
				python runs/leds.py $ledstandby &
		...
		if (( lademodus == 0 )); then
			if [[ $ledstatus != $led0sofort ]]; then
				...
				python runs/leds.py $led0sofort &
		...
		if (( lademodus == 4 )); then
			if [[ $ledstatus != $led0standby ]]; then
				...
				python runs/leds.py $led0standby &
